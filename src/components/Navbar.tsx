...
export function Navbar() {
  ...
  <Link to="/contact" className="nav-link-underline py-1 hover:text-brand-accent transition-colors">Contactez-nous</Link>
</div>
          </div>

          {/* Search Bar in Middle */}
          <div className="hidden lg:block relative w-[480px] group mx-8">
  <input 
    type="text" 
    placeholder="lawej 7ajtek" 
    value={searchQuery}
    onChange={handleSearch}
    className="w-full bg-white border-2 border-brand-accent/40 rounded-3xl px-6 py-3 text-brand-ink placeholder:text-brand-accent/60 focus:outline-none focus:border-brand-accent focus:ring-4 focus:ring-brand-accent/10 shadow-md text-base normal-case tracking-normal transition-all duration-300"
  />
...
<Link to="/contact" onClick={() => setIsMobileMenuOpen(false)} className="hover:bg-brand-accent/10 hover:text-brand-accent transition-colors px-4 py-3 rounded-lg">Contactez-nous</Link>
