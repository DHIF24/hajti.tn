...
export const ProductCard: React.FC<{ product: Product; listView?: boolean; badge?: { text: string; color: string } }> = ({ product, listView, badge }) => {
  ...
  const hasPromotion = Number(product.promotionPercentage) > 0;
const originalPrice = hasPromotion ? product.price / (1 - Number(product.promotionPercentage) / 100) : product.price;
  ...
  if (listView) {
    return (
      ...
      >
        <Link to={`/product/${product.id}`} className="group flex flex-col sm:flex-row gap-8 items-center bg-transparent p-0 transition-all duration-300">
          <div className="relative w-full sm:w-1/3 aspect-square overflow-hidden bg-[#f8f8f8] rounded-2xl flex-shrink-0">
            ...
            />
            {hasPromotion && product.promotionPercentage > 0 && (
